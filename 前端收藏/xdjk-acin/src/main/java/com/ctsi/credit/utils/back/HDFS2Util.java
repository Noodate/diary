package com.ctsi.credit.utils.back;
...
import com.ctsi.credit.utils.ConvertUtils;
import com.ctsi.credit.utils.FileUtils;
import com.ctsi.credit.utils.ImageUtils;
import com.ctsi.credit.utils.VideoUtils;

/**
 * HDFS操作类  
 */
public class HDFS2Util {

	private HDFS2Util() {
	}
	static Logger logger=Logger.getLogger(HDFS2Util.class);
	// hadoop fs的配置文件
	static Configuration conf = new Configuration(true);
//	static String HADOOP_URL = "hdfs://10.13.141.24:9000";
	static String HADOOP_URL = "hdfs://localhost:8010";
	static String HADOOP_USER_NAME = "xdjk";
	static {
		// 指定hadoop fs的地址
		conf.set("fs.hdfs.impl", "org.apache.hadoop.hdfs.DistributedFileSystem");
		// conf.set("fs.default.name", "hdfs://localhost:8010");
	...
	public static boolean exits(String path) throws IOException {
		...
		try {
			...
			exists = fs.exists(new Path(HADOOP_URL + path));
	...
	public static void copyFromLocalFile(String localFilePath, String remoteFilePath) throws IOException {
		try {
			FileSystem fs = FileSystem.get(URI.create(HADOOP_URL), conf, HADOOP_USER_NAME);
			Path localPath = new Path(localFilePath);
			Path remotePath = new Path(HADOOP_URL + remoteFilePath);
	...
	public static boolean deleteFile(String remoteFilePath, boolean recursive) throws IOException {
		...
		try {
			fs = FileSystem.get(URI.create(HADOOP_URL), conf, HADOOP_USER_NAME);
			result = fs.delete(new Path(HADOOP_URL + remoteFilePath), recursive);
			fs.close();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
