...
//     用 html页面测试，返回值不成功？？？ test.html
    @RequestMapping(value = "/cookie2", method = RequestMethod.POST)
    @ResponseBody
    public String login2( HttpServletResponse response, User user) {
// html 测试用
         response.setHeader("Access-Control-Allow-Origin", "*");
...
//        response.setHeader("Access-Control-Allow-Headers", "x-requested-with,Authorization");
        ...
        if (resultUser == null) {
            jsonObject = JSONObject.fromObject(ResultGenerator.genFailResult("请认真核对账号、密码！"));
        } else {
            resultUser.setPassword("PASSWORD");
            Map data = new HashMap();
            data.put("currentUser", resultUser);
           jsonObject = JSONObject.fromObject(ResultGenerator.genSuccessResult(data));
        }
